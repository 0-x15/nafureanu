...
import { KICKER } from "./csBits";
import CsHeroVisual from "./CsHeroVisual";

/** Hero — from an unclear need to software in production. */
export default function CsHero({ lang, c, proofPath }) {
  ...
  return (
    <header id="cs-hero" className="mx-auto grid max-w-[1440px] items-center gap-12 overflow-x-clip px-5 pb-12 pt-8 md:px-10 md:pb-16 md:pt-12 lg:grid-cols-12 lg:items-start lg:gap-8">
      ...
      <Reveal eager variant="scale" delay={0.1} className="min-w-0 md:mx-auto md:w-full md:max-w-[620px] lg:col-span-7 lg:max-w-none lg:pl-4 lg:pt-1">
        <CsHeroVisual h={h} />
      </Reveal>
    </header>
  );

  ...
  return (
    <section className="px-5 pb-24 md:px-10 md:pb-36" aria-labelledby="process-title">
      <p className="font-mono text-[11px] uppercase tracking-[0.25em] text-[#848482]">
        <span className="text-[#E63946]">{s.processKicker.split(" — ")[0]}</span> —{" "}
      ...
      <div className="mt-12 grid border-t border-[#E0E0DE] sm:grid-cols-2 lg:grid-cols-6">
        {steps.map((step, i) => (
          <Reveal
            key={step.num}
            delay={i * 0.06}
            className="border-b border-[#E0E0DE] py-8 sm:border-l sm:first:border-l-0 sm:px-5 sm:first:pl-0 lg:border-b-0"
          >
            <p className="font-mono text-[11px] text-[#E63946]">{step.num}</p>
            <h3 className="mt-3 font-heading text-lg font-bold uppercase tracking-[-0.01em]">
              {step.title}
            </h3>

...
import { KICKER } from "./bsBits";
import BsHeroVisual from "./BsHeroVisual";

/**
 * Hero — the company's model becomes software. Copy left; on the right
 * the model resolving in four steps: what exists, how it relates, what
 * can happen, one system.
 */
export default function BsHero({ lang, c, proofPath }) {
  const h = c.hero;
  return (
    <header id="bs-hero" className="mx-auto grid max-w-[1440px] items-center gap-12 overflow-x-clip px-5 pb-12 pt-8 md:px-10 md:pb-16 md:pt-12 lg:grid-cols-12 lg:gap-8">
      ...
      <Reveal eager variant="scale" delay={0.1} className="min-w-0 md:mx-auto md:w-full md:max-w-[620px] lg:col-span-7 lg:max-w-none lg:pl-4">
        <BsHeroVisual h={h} />
      </Reveal>
    </header>
  );

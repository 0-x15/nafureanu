...
export default function WorkRange({ t }) {
  ...
  const r = t.range;
  return (
    <section aria-labelledby="work-range" className="relative overflow-hidden bg-background px-5 py-20 md:px-10 md:py-28">
      ...
      <div className="relative mx-auto max-w-[1440px]">
        <div className="grid gap-10 lg:grid-cols-12 lg:gap-x-10">
          ...
          <div className="relative hidden aspect-[16/9] lg:col-span-8 lg:block">
            ...
            {/* axis labels */}
            <p className={cn(MONO, "absolute left-0 top-1/2 -translate-y-1/2 bg-background pr-3 text-foreground/70")}>{r.axisX[0]}</p>
            <p className={cn(MONO, "absolute right-0 top-1/2 -translate-y-1/2 bg-background pl-3 text-right text-foreground/70")}>{r.axisX[1]}</p>
            <p className={cn(MONO, "absolute left-1/2 top-0 -translate-x-1/2 bg-background px-3 text-foreground/70")}>{r.axisY[0]}</p>
            <p className={cn(MONO, "absolute bottom-0 left-1/2 -translate-x-1/2 bg-background px-3 text-foreground/70")}>{r.axisY[1]}</p>
            ...
            {r.items.map((item, i) => {
              ...
              return (
                ...
                >
                  <span aria-hidden="true" className="absolute -left-3 top-[0.55em] h-[7px] w-[7px] border border-accent bg-background" />
                ...
                return (
                  <li key={item.slug} className="border-b border-foreground/12 py-4">
                    <div className="relative h-px bg-foreground/12">
                      <span aria-hidden="true" className="absolute top-1/2 h-[7px] w-[7px] -translate-y-1/2 border border-accent bg-background" style={{ left: `calc(${p.x}% - 3px)` }} />
                    </div>

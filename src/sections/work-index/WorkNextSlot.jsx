...
export default function WorkNextSlot({ lang, t, projects }) {
  const n = t.next;
  return (
    <section aria-labelledby="work-next" className="relative overflow-hidden border-t border-foreground/10 px-5 py-20 md:px-10 md:py-28">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        <span className="absolute -bottom-[30%] left-[30%] h-[90%] w-[60%] rounded-full bg-[radial-gradient(closest-side,rgba(49,87,246,0.07),transparent)]" />
      </div>
            ...
            <ol className="mt-3 border-t border-foreground/15">
              {projects.map((p, i) => (
                <li key={p.slug} className="border-b border-foreground/12">
                  <Link to={langPath(lang, `/work/${projectSlug(p, lang)}`)} className="group grid grid-cols-[2.5rem_1fr] items-baseline gap-x-4 py-3 outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-4 focus-visible:ring-offset-[#F9F7F0]">
              ...
              <li className="border-b border-accent/40">
                <div tabIndex={0} aria-label={n.blank} className="group grid grid-cols-[2.5rem_1fr] items-baseline gap-x-4 py-4 outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-4 focus-visible:ring-offset-[#F9F7F0]">
                  ...
                  <span className="relative block">
                    ...
                    <span aria-hidden="true" className="relative mt-[0.55em] block h-px w-[52%] bg-foreground/35 transition-[width] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:w-full group-focus-visible:w-full">
                      <span className="absolute -right-px -top-[4px] h-[9px] w-[9px] border border-accent bg-[#F9F7F0] opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-visible:opacity-100" />
                    </span>

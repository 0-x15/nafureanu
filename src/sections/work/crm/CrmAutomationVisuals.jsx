...
const TONES = {
  ...
  accent: "border-[#3157F6]/60 bg-[#3157F6]/15 text-[#BFD0FF]",
  cyan: "border-[#17B4CD]/50 bg-[#17B4CD]/10 text-[#9FE6F0]",
  warn: "border-[#D9A441]/50 bg-[#D9A441]/10 text-[#F0D08A]",
  attention: "border-[#E3735E]/50 bg-[#E3735E]/10 text-[#F5B3A6]",
};

function Surface({ className = "", children }) {
  return (
    <div className={cn("rounded-md border border-white/10 bg-[#0F182B] p-3.5 md:p-4", className)}>
      {children}
    </div>
  );
}
...
function Label({ className = "", children }) {
  return (
    <p className={cn("font-mono text-[9px] uppercase tracking-[0.14em] text-white/40", className)}>
      {children}
    </p>
  );
}
    ...
    >
      <span
        className={cn(
          ...
          lineClass,
          on ? "bg-[#3157F6]/70" : "bg-white/15"
        )}
      />
      {pulse && (
        <motion.span
          className={cn(
            "absolute h-1.5 w-1.5 rounded-full bg-[#5B84FF] shadow-[0_0_6px_rgba(91,132,255,0.8)]",
  ...
  return (
    <motion.span
      ...
      className={cn(
        ...
        done ? "border-[#3157F6] bg-[#3157F6]" : "border-white/25 bg-transparent"
      )}
    >
      ...
      <div className={flowClass}>
        ...
        <span className="flex flex-col gap-1.5">
          <Node tone="accent" on={phase >= 1}>
            <span className="h-1 w-1 rounded-full bg-[#5B84FF]" />
            {ui.engine}
          </Node>
        ...
        <ul className="grid grid-cols-2 gap-x-5 gap-y-1.5">
          {ui.criteria.map((criterion, i) => (
            <li key={criterion} className="flex items-center gap-1.5 text-[10.5px] text-white/75">
...
function LeadsVisual({ ui, active, reduce, lane }) {
  ...
  return (
    <Surface>
      ...
      <p className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-white/[0.08] pt-2.5 font-mono text-[8.5px] uppercase tracking-[0.12em] text-white/40">
        <span>{ui.noContact}</span>
        <span className="text-white/25">→</span>
        <span className={cn("transition-colors duration-300", phase >= 3 && "text-[#F0D08A]")}>
          ! {ui.alert}
        </span>
        <span className="text-white/25">→</span>
        <span>{ui.queue}</span>
          ...
          {ui.items.map(([label, done], i) => {
            ...
            return (
              <li key={label} className="flex items-center gap-2 text-[11px]">
                <Check done={isDone} />
                <span className={isDone ? "text-white/80" : "text-white/45"}>{label}</span>
                {auto && (
                  <motion.span
                    animate={{ opacity: phase >= 3 ? 1 : 0 }}
                    transition={{ duration: 0.3 }}
                    className="ml-1 font-mono text-[8px] uppercase tracking-[0.12em] text-[#5B84FF]"
                  >
                    {ui.autoTag}
                  </motion.span>
                )}
...
const STATE_GLYPH = {
  ok: "border-[#17B4CD]/50 bg-[#17B4CD]/10 text-[#9FE6F0]",
  warn: "border-[#D9A441]/50 bg-[#D9A441]/10 text-[#F0D08A]",
  attention: "border-[#E3735E]/50 bg-[#E3735E]/10 text-[#F5B3A6]",
};
  ...
  return (
    <Surface>
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="flex items-center gap-2 font-mono text-[8.5px] uppercase tracking-[0.12em] text-[#8FA5E8]">
      ...
      <ul className="mt-2 divide-y divide-white/[0.06]">
        {ui.items.map(([label, tone], i) => (
          <motion.li
            ...
            transition={{ duration: 0.35, ease: EASE }}
            className="flex items-center justify-between gap-3 py-1.5 text-[11px] text-white/75"
          >
            <span>{label}</span>
            <span
              className={cn(
    ...
    <Surface className="space-y-3">
      {rows.map((row, r) => (
        <div
          ...
          className={cn(
            flowClass,
            r > 0 && "border-t border-white/[0.06] pt-3 sm:border-0 sm:pt-0"
          )}
        >
  ...
  return (
    <Surface className="flex flex-col gap-3 md:grid md:grid-cols-[minmax(0,1.05fr)_auto_minmax(0,1fr)] md:items-center md:gap-5">
      <div>
        ...
        <p className="mt-2 rounded-md rounded-tl-sm border border-white/10 bg-white/[0.04] px-3 py-2 text-[11.5px] leading-snug text-white/80">
          “{ui.message}”
        </p>
      </div>
        ...
        <dl className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-1">
          {ui.fields.map(([key, value], i) => (
            <motion.div
              ...
              className="contents"
            >
              <dt className="font-mono text-[8.5px] uppercase tracking-[0.12em] text-white/40">
                {key}
              </dt>
              <dd className="border-b border-white/[0.06] pb-1 text-[11px] text-white/85">{value}</dd>
            </motion.div>
          ))}
        </dl>
